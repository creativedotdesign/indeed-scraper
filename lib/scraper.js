...
function Query(qo) {
  ...
  this.excludeAds = qo.excludeAds || false;
...
Query.prototype.getJobs = function() {
  ...
  const excludeAds = this.excludeAds;
...
function parseJobList(body, host, excludeSponsored, excludeAds) {
  ...
  filtered =
    excludeAds === "true"
      ? jobs.filter((_, e) => {
          const job = $(e);
          const href = job.find(".jobtitle").attr("href");
          const isAd = href.includes("/pagead/");
  ...
  const jobObjects = filtered
    .map((i, e) => {
      ...
      // Format date time
      postDate = tr.parse(postDate);

      if (postDate) {
        postDate = dateFormat(postDate, "yyyy-mm-dd HH:MM:ss");
      }
